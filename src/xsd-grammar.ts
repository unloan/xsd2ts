...
const fieldHandler: AstNodeFactory = (n) =>
  attribs(n).type
...
const topFieldHandler: AstNodeFactory = (n) => {
  ...
  return attribs(n).type || attribs(n).abstract
    ? astNode("AliasType").addAttribs(n).prop("element", "true")
    : null;
  //return /\w:/.test(attribs(n).type) ? astNode('AliasType').addAttribs(n) : null;
};

const attrHandler: AstNodeFactory = (n) =>
  astNode("Field").addField(n).prefixFieldName("$");

const arrayFldHandler: AstNodeFactory = (n) =>
  attribs(n).type && attribs(n).maxOccurs === "unbounded"
    ? astNode("ArrField").addField(n).prop("label1", "arrayFldHandler")
    : null;

const cmpFldHandler: AstNodeFactory = (n) =>
  astField()
    .prop("label2", "cmpFldHandler")
    .addField(n, capFirst(attribs(n).name));

const classHandler: AstNodeFactory = (n) =>
  attribs(n).type ? null : astClass(n).prop("label3", "classHandler");
const classElmHandler: AstNodeFactory = (n) =>
  attribs(n).type
    ? null
    : astClass(n).prop("label3", "classElmHandler").prop("element", "true");

const namedUntypedElmHandler: AstNodeFactory = (n) =>
  attribs(n).type || !attribs(n).name
    ? null
    : astNamedUntypedElm(n).prop("element", "true");
const enumerationHandler: AstNodeFactory = (n) =>
  attribs(n).value ? astEnumValue(n) : null;
const restrictionHandler: AstNodeFactory = (n) =>
  attribs(n).value ? astRestrictions(n) : null;

const extensionHandler: AstNodeFactory = (n) =>
  astNode("Extension").addAttribs(n);

const nrRestrictionHandler: AstNodeFactory = (n) =>
  numberRegExp.test(attribs(n).base)
    ? astNode("AliasType").prop("value", "number")
    : null;
const strRestrictionHandler: AstNodeFactory = (n) =>
  /string/.test(attribs(n).base)
    ? astNode("EnumOrAliasType").prop("value", "string")
    : null;
const dtRestrictionHandler: AstNodeFactory = (n) =>
  /(dateTime|date)/.test(attribs(n).base)
    ? astNode("AliasType").prop("value", "Date")
    : null;

const namedGroupHandler: AstNodeFactory = (n) =>
  attribs(n).name ? astNode("Group").named(attribs(n).name) : null;
const namedSimpleTypeHandler: AstNodeFactory = (n) =>
  attribs(n).name ? astNode("SimpleType").named(attribs(n).name) : null;
const refGroupHandler: AstNodeFactory = (n) =>
  attribs(n).ref ? astNode("Fields").prop("ref", attribs(n).ref) : null;
const refElementHandler: AstNodeFactory = (n) =>
  attribs(n).ref ? astNode("Reference").addAttribs(n) : null;

const childsMerger: AstNodeMerger = (r1, r2) => {
...
const ccontSeqAttrMerger: AstNodeMerger = (r1, r2) => {
  r2.children?.forEach((c) => {
    ...
    if (c.nodeType === "complexContent") {
      ...
      if (r1.children) r1.children.push(c);
      else r1.children = [c];
...
const enumMerger: AstNodeMerger = (r1, r2) => {
  r1.nodeType = "Enumeration";
  r1.attr.values = r2.children;
  return r1;
};
const typeMerger: AstNodeMerger = (r1, r2) => {
  r1.nodeType = "AliasType";
  r1.attr.type = r2.attr.value;
  return r1;
};
...
const nestedClassMerger: AstNodeMerger = (r1, r2) => {
  r1.nodeType = "Field";
  r1.attr.nestedClass = { name: r1.attr.fieldType, children: r2.children };
  return r1;
};
const arrayFieldMerger: AstNodeMerger = (r1, r2) => {
  r2.nodeType = "Field";
  r2.attr.fieldName = r1.attr.fieldName;
  return r2;
};
...
export class XsdGrammar {
  ...
  public parse(node: Node): ASTNode {
    ...
    const eNamedUntyped = new Terminal(
      "element:namedUntypedElm",
      namedUntypedElmHandler
    );

    const attributeGroup = new Terminal(
      "attributeGroup:attrGrp",
      namedGroupHandler
    );
    const schema = new Terminal(
      "schema:Schema",
      makeSchemaHandler(this.schemaName)
    );
    ...
    const strRestriction = new Terminal(
      "restriction:strRestr",
      strRestrictionHandler
    );
    const nrRestriction = new Terminal(
      "restriction:nrRestr",
      nrRestrictionHandler
    );
    const dtRestriction = new Terminal(
      "restriction:dtRestr",
      dtRestrictionHandler
    );
    ...
    const ARRFIELD = match(cmpFldElement, arrayFieldMerger)
      .child(complexType)
      .child(sequence)
      .child(arrFldElement)
      .labeled("ARRFIELD");

    const CMPFIELD = match(cmpFldElement, nestedClassMerger)
      .child(complexType)
      .child(sequence)
      .children(FIELDPROXY)
      .labeled("CMPFIELD");
    ...
    const A_CLASS = match(classElement, childsMerger)
      .child(complexType)
      .children(ATTRIBUTE, CHOICE, ATTREFGRP)
      .labeled("A_CLASS");
    // element class
    const E_CLASS = match(classElement)
      .child(complexType)
      .child(sequence, childsMerger)
      .children(FIELD)
      .labeled("E_CLASS");
    ...
    const G_CLASS = match(attributeGroup)
      .children(match(attribute))
      .labeled("G_CLASS");

    // coplex type class
    const SEQUENCE = match(sequence, childsMerger)
      .children(FIELD)
      .labeled("SEQUENCE");
    ...
    const R_CLASS = match(classType, childsMerger)
      .children(REFGROUP, ATTRIBUTE)
      .labeled("R_CLASS");
    //const C_CLASS  = match(classType).childIsOneOf(SEQUENCE, CCONTENT).labeled('C_CLASS');
    const C_CLASS = match(classType, ccontSeqAttrMerger)
      .children(SEQUENCE, CCONTENT, REFGROUP, ATTRIBUTE)
      .labeled("C_CLASS");
    ...
    const N_GROUP = match(namedGroup)
      ...
      .children(FIELD)
      .labeled("N_GROUP");
    const ENUMELM = match(eNamedUntyped, enumMerger)
      .child(simpleType)
      .child(strRestriction)
      .children(match(enumeration))
      .labeled("ENUMELM");
    const ENUMTYPE = match(namedSimpleType, enumMerger)
      .child(strRestriction)
      .children(match(enumeration))
      .labeled("ENUMTYPE");
    const SRESTR = oneOf(
    ...
    const ALIAS1 = match(namedSimpleType, typeMerger)
      .child(dtRestriction)
      .labeled("ALIAS1");
    const ALIAS2 = match(namedSimpleType, patternChildrenMerger)
      .child(nrRestriction, childsMerger)
      .children(NRESTR)
      .labeled("ALIAS2");
    const ALIAS3 = match(namedSimpleType, patternChildrenMerger)
      .child(strRestriction, childsMerger)
      .children(SRESTR)
      .labeled("ALIAS3");
    const ALIAS4 = match(eNamedUntyped, patternChildMerger)
      .child(simpleType, patternChildrenMerger)
      .child(strRestriction, childsMerger)
      .children(SRESTR)
      .labeled("ALIAS4");
    const ALIAS5 = match(eNamedUntyped, patternChildMerger)
      .child(simpleType, patternChildrenMerger)
      .child(nrRestriction, childsMerger)
      .children(NRESTR)
      .labeled("ALIAS5");
